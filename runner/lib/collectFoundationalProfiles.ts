...
async function importSlabProfiles(
  profilesDir: string,
  userAgentConfig: UserAgentConfig,
  options: CollectFoundationalProfilesOptions,
) {
  ...
  if (!slabDataDir) {
    if (options && options.slabDataDir) {
      ...
      throw new Error(
        'no slab data dir defined in options or env, while it is required for collecting foundational profiles from',
      );
    }
  }
