...
async function runFactoryRunners(runnerFactory: IRunnerFactory, runnerID: string, userAgentsToTestPath: string, assignmentsDataOutDir: string, concurrency: number) {
...
async function runAssignments(runnerFactory: IRunnerFactory, userAgentsToTestPath: string, assignmentsDataOutDir: string, options?: RunAssignmentsOptions) {
    ...
    try {
        await runFactoryRunners(runnerFactory, runnerID, userAgentsToTestPath, assignmentsDataOutDir, options.concurrency);
    } catch (error) {
